...
export class OtherPlayer extends UpdatePlayer {
    ...
    update(delta_time: number, now_time: number) {
        ...
        if (this.game.on_podium(pos)) {
            ...
            const l_table = this.game.at_table(l_pos);
            const my_table = this.game.at_table(pos);
            if (l_table == '' || my_table == '' || l_table == my_table) {
                this.panner.positionX.value = pos.x;
                this.panner.positionZ.value = pos.y;// z is the new y
            } else { // the listener sits at a dedicated table different to this player
                // if listener sits at a table all that sit on another table are muted
                //TODO actually we shouldn't just control the pannernode but the stereopanner
                // we actually want to mute it here: ugly, but should do the job
